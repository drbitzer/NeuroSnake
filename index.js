...
function draw() {
    ...
    if (snakeArr.length == 0) {      
      ...
      //Search for the best eater Snake
      for (let i = 0; i < snakeSave.length; i++) {
        fitness = snakeSave[i].apples;
      ...
      //Search for the best long lasting Snake
      fitness = 0;
      bestFitness = 0;
      for (let i = 0; i < snakeSave.length; i++) {
        fitness = snakeSave[i].score;
        if (fitness > bestFitness) {
      ...
      //Add again 2 best Snakes to the next generation
...
function calcFitness() {
  ...
  snakeSave.forEach(element => {
    element.fitness = ( element.apples + element.score ) / totalScore;    
  });
}
