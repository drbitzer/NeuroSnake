...
function setup() {
  ...
  starBtn.position(0, 550);
  starBtn.mousePressed(start);  

  drawBtn = createButton('Set Draw');
  drawBtn.position(0, 580);
  drawBtn.mousePressed(toggleDraw);

  loadBtn = createInput('', 'file');
  loadBtn.position(0, 610);
  loadBtn.input(loadBest);  

  saveBtn = createButton('Save Best');
  saveBtn.position(0, 640);
  saveBtn.mousePressed(saveBest);
...
function loadBest(event) {
  snakeArr = [];  

  let reader = new FileReader();
  reader.onloadend = function() {
    savedSnake = reader.result;
  };
...
function saveBest() {
  saveJSON(bestSnakeEver, 'bestSnakeEver_gen' + bestSnakeEverGen + '.json');
}
...
/*       while (true) {        
        ...
        parent1 = snakeSave[index];

        randomFitness = random(maxFitness);
        if (randomFitness <= parent1.fitness)
          break;      
      }  
      ...
      while (true) {
        index = floor(random(snakeSave.length));
        parent2 = snakeSave[index];

        randomFitness = random(maxFitness);
        if (randomFitness <= parent2.fitness)
          break;      
      }   */        
...
/* 	  let bestSnake1 = snakeSave[0];      
      ...
      avgFitness = (bestSnake1.fitness + bestSnake2.fitness) / 2;
      fitnessElem.html('Avg. Fitness = ' + avgFitness.toFixed(2) + ' - Best = ' + bestSnake1.fitness.toFixed(2));	   */
...
/*         while(!parent1) {
        ...
        snakeArr.push(new Snake(500, 500, 10, parent1));
        snakeArr[i].brain.combine(parent2.brain);
        snakeArr[i].brain.mutate(0.3);
        snakeArr[i].setApple(new Apple(500, 500, 10));
        snakeArr[i].apple.spawn();
...
/*         if (parent1.fitness > bestFitness) bestFitness = parent1.fitness;
      ...
      //Check and save the best snake ever
      if (bestSnakeEver && parent1.fitness > bestSnakeEver.fitness) {
        bestSnakeEver = parent1;
        bestSnakeEverGen = generation - 1;
      } else if (!bestSnakeEver) {
        bestSnakeEver = parent1;
        bestSnakeEverGen = generation - 1;
      }
    }     
